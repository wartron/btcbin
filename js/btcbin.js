...
var btcbin = {
	...
	init: function(){
		...
		$(".langselector a").on('click',function(e){
			nlang = $(this).attr("lang");
			if(btcbin.settings.o.defaultLang != nlang){
				btcbin.settings.o.defaultLang = nlang;
				console.log(nlang);
				btcbin.localizerLoad(nlang);
				//btcbin.localize(); //this should now be trrgger from localizerLoad
			}
...
*/
		...
		if(btcbin.settings.o.defaultLang != 'en'){
			//btcbin.localizerLoad(btcbin.settings.o.defaultLang);
		}
	...
	localizerLoad: function(lang){
		// if($.inArray(lang, btcbin.loaded_lang)){
		// 	btcbin.localize();
		// 	return
		// }

		var url = 'js/strings/'+lang+'.js'
		$.ajax({
			url: url,
			dataType: 'script',
			cache: false,
			success: function(script, textStatus) {
		 		btcbin.localize();
		 		//btcbin.loaded_lang.push(lang);
			}
		});
	},
	localtxt: function(id){
		var t = 'oops'
		if(strings[btcbin.settings.o.defaultLang]){
			t = strings[btcbin.settings.o.defaultLang][id];
			if(t)
				return t;
		}
		t = strings["en"][id];
